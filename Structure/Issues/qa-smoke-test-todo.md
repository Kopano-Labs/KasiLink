...
1. Clean up `/forum` first.
- Fix the `handlePost` indentation/syntax area.
- Add better validation feedback before posting.
- Confirm the page still posts and paginates after the cleanup.

2. Refactor `/utility-schedule`.
- Remove render-time `Date.now()` dependency from `next` selection.
...
5. Tighten `/marketplace`.
- Reduce filter complexity where possible.
- Add clearer empty-state messaging.

6. Split cramped profile/chat pages later.
- Separate list and detail concerns into smaller components.
- Keep the existing behavior, but reduce maintenance risk.
